...
class PDF extends FPDF
{
    function Header() {
        ...
        $this->Cell(0,10,'Yearly Profit & Loss Report',0,1,'C');
        $this->Ln(5);
    }
...
$expenseData = select("SELECT Name, TotalAmount FROM Expenses WHERE YEAR(Date) = ?", [$year]);
$totalExpense = 0;

$salesData = select("SELECT Name, AmountPaid FROM Clients WHERE YEAR(StartDate) = ? AND IsDelete = 1", [$year]);
$totalSales = 0;
